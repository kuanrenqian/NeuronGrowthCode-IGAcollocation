function [Jm,Coeff,Pixel] = kqConstructAdaptiveGrid(ac,parameters,Dm,Em,THBfinal,knotvectorU,knotvectorV,multilev,nobU,nobV,nelemU)
...
for i = 1:length(THBfinal)
    p_ind = p_ind+1;
    uu = FindSpan(unobMAX-1,pU,THBfinal(i,2),kuMAX) + 1;
    vv = FindSpan(vnobMAX-1,pV,THBfinal(i,1),kvMAX) + 1;
    ...
    if(EE{cell_ind,4} == 1)
        ...
        RRD1 = dersbasisfuns3(uu,pU,2,THBfinal(i,2)*40,kuMAX*40);
        RRD2 = dersbasisfuns3(vv,pV,2,THBfinal(i,1)*40,kvMAX*40);
%             RRD1 = dersbasisfuns3(uu,pU,2,THBfinal(i,1),kuMAX);
%             RRD2 = dersbasisfuns3(vv,pV,2,THBfinal(i,2),kvMAX);
        ...
        for m = (multilev+1):-1:2
            ...
            if(EEMM{curr_cell,4} == 1)
                ...
                unob1 = nobU(m-1,1);
                vnob1 = nobV(m-1,1);
                u1 = FindSpan(unob1-1,pU,THBfinal(i,2),knotuu) + 1;
                v1 = FindSpan(vnob1-1,pV,THBfinal(i,1),knotvv) + 1;
                pix_coeff = Coeff{act_ind,1};
                SB1 = EE{cell_ind,6};
                sb_size1 = size(SB1,1);

                RRD1 = dersbasisfuns3(u1,pU,2,THBfinal(i,2)*40,knotuu*40);
                RRD2 = dersbasisfuns3(v1,pV,2,THBfinal(i,1)*40,knotvv*40);
%                     RRD1 = dersbasisfuns3(uu,pU,2,THBfinal(i,1),kuMAX);
%                     RRD2 = dersbasisfuns3(vv,pV,2,THBfinal(i,2),kvMAX);
