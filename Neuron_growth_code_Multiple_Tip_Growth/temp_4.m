...
%     if (iter>=rot_iter_start||expd_state==1)
    if (iter>=rot_iter_start)        
...
%                 % use initial angle to grow out (some conversion needed for
%                 % rotate variable)
%                 if(initial_angle>0)
%                     rotate = pi-initial_angle;
%                 else
%                     rotate = -(pi+initial_angle);
%                 end
                ...
                for i = 1:size_Max
                    Rot = rand*2-1;
                    rotate(i) = Rot;
                % check if Rot is in range, or calculate Rot again
%                 pass = 0;
%                     while (pass==0)
%                         if (initial_angle>0)
%                             if ( (rotate+Rot)>initial_up_range || ...
%                                     (rotate+Rot)<initial_down_range )
%                                         Rot = rand*pi-pi/2;
%                                         pass = 1;
%                             end
%                         elseif (initial_angle<0)
%                             if ( (rotate+Rot)<initial_up_range || ...
%                                     (rotate+Rot)>initial_down_range )
%                                         Rot = rand*pi-pi/2;
%                                         pass = 1;
...
%                     end
        ...
        % x,y naming issue
        theta_ori = theta_rotate(lenu,lenv,Max_x,Max_y,initial_angle,rotate,size_Max);
