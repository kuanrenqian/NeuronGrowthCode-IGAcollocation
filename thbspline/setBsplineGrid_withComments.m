...
% Cell array - control points at different level
% lev 123.. 
CP = cell(maxlev,1);

% knot vector - u (each level - maxlev)
knotvectorU = cell(maxlev,1);
% nelemU - corresponding element size
nelemU = zeros(maxlev,1);
% unique knots
uknotvectorU = cell(maxlev,1);
% size of each knot vector - each level
kU = zeros(maxlev,1);
% number of basis fucntions = number of control points
nobU = zeros(maxlev,1);
...
for lev = 1:maxlev,
    ...
    for intU = 1:nobU(lev,1),
        for intV = 1:nobV(lev,1),
            ...
            if((intU>=(pU+1))&&(intV>=(pV+1))),
                ...
                % knot vector index in x
                connectU(1,1) = intU;
                connectU(2,1) = connectU(1,1)+1;
                
                % knot vector index in y
                connectV(1,1) = intV;
                ...
                % children cell - next level refinement cells
                EEN{E,5} = ch;
                ...
                % number of non-zero spline over each element
                EEN{E,6} = BBvector;
    ...
    for basis_i =1:nobU(lev,1),
        for basis_j = 1:nobV(lev,1),
            ...
            % supporting cell of each basis function
            ...
            % active basis function index
            Basis_m{bc,7} = 0;
            for iU = 1:nobU(lev,1),
                dof1 = iU; %left most bc
                Basis_m{dof1,9} = 1;
                dof2 = iU + nobU(lev,1)*(nobV(lev,1)-1); %right most bc
                Basis_m{dof2,9} = 1;
            end
            
            for iV = 1:nobV(lev,1),
                dof3 = 1+nobU(lev,1)*(iV-1); % top 
                Basis_m{dof3,9}  =1;
                dof4 = nobU(lev,1)+nobU(lev,1)*(iV-1); % bottom
                Basis_m{dof4,9}  =1;
            end
            if(lev<=maxlev-1)
                ...
                % Chirldren spline index 
                Basis_m{bc,4} = C;
                % Coefficient matrix - for truncation
                Basis_m{bc,12} = Coeff;
...
for i = 1:nobU(1,1),
    for j =1:nobV(1,1),
        ...
        % greville absce - collocation points
        % open knot domain - obtain same domain position back 
