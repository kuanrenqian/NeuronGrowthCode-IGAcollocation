...
delta = 0.2;
epsilonb = 0.1;
% epsilonb = epsilonb.*ones(lenu*lenv,1); % anisotropic
% delta = delta.*ones(lenu*lenv,1);
...
%% Start video writer
writer = VideoWriter('NeuronGrowth_0201_kappa2'); % Define VideoWriter object
open(writer);
...
epb_scale = ones(lenu,lenv);
for i=1:lenu
    for j=1:lenv
        if ((i-lenu/2)*(i-lenu/2)+(j-lenv/2)*(j-lenv/2) < 15^2)
            epb_scale(i,j) = 0;
        end
    end
end
epb_scale = reshape(epb_scale,lenu*lenv,1);
...
%     epsilonb = 0.07 + reshape(param.H,lenu*lenv,1).*0.13.*epb_scale;
    ...
    E = (alph./pix).*atan(gamma.*(teq-abs(NuNv*tempr)));       
...
%         fprintf('Phi NR Iter: %.2d -> max residual: %.2d\n',ind_check, max_phi_R);
    ...
    tempr_new = NuNv\(NuNv*tempr + Dt.*lap*tempr.*dtime_tempr + kappa*(NuNv*phiK-NuNv*phi));
...
%         colorbar;
% 
% %         epsilonb_plot = reshape(epsilonb,lenu,lenv);
% %         subplot(4,2,4);
% %         imagesc(epsilonb_plot);
% %         title(sprintf('epsilonb at iteration = %.2d',iter));
% %         axis square;
% %         colorbar;
% % 
% %         epb_scale_plot = reshape(epb_scale,lenu,lenv);
% %         subplot(4,2,5);
% %         imagesc(epb_scale_plot);
% %         title(sprintf('epb_scale at iteration = %.2d',iter));
% %         axis square;
% %         colorbar;
% 
...
% %         colorbar;
        ...
        %% 
%         a = reshape(smooth(full(phi_plot)),lenu,lenv);
%         for i = 1:lenu^2
%             if(a(i)>0.1)
%                 a(i) = 1;
%             else
