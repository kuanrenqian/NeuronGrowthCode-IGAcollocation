...
% reduce time step by half
% 0.5e-2
% 1e-3
% 2e-2
% 5e-3
% 2.5e-3
% check other literature time steps
% run to correct time steps (based on dt)
% new governing eqn
% dt*time_step
...
%% Start video writer
writer = VideoWriter('NeuronGrowth_ex3'); % Define VideoWriter object
open(writer);
...
% %     end
    ...
    if(mod(iter,5) == 0 || iter == 1)
        ...
        diff_plot = reshape(gamma.*(teq-(NuNv*tempr)),lenu,lenv);
        subplot(5,2,7);
        imagesc(diff_plot);
        title(sprintf('teq-T at iteration = %.2d',iter));
        axis square;
        colorbar;
...
%         
        ...
        saveas(gcf,sprintf('NeuronGrowth_ex%.2d.png',iter));
        ...
        if(iter~=1)
            if( max(max(phi_plot(1:BC_tol,:))) > 0.5 || ...
                ...
                sz = length(lap);
                [phi,theta,conc,tempr,phi_initial,theta_initial,conc_initial,tempr_initial,bcid] ...
...
% 
% %% Start video writer
% writer = VideoWriter('NeuronGrowth_ex2'); % Define VideoWriter object
% open(writer);
% 
% phi_actin = reshape(NuNv*phi,lenu,lenv);
% param = GetParam(phi_actin);
