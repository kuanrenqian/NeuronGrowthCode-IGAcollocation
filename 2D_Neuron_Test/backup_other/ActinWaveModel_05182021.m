...
u_initial = reshape(u,lenu*lenv,1);
v_initial = reshape(v,lenu*lenv,1);
...
figure(1);
set(gcf,'position',[900,100,500,700]);
...
%     u_new = NuNv\(NuNv*u + dt*duT.*P_temp);
%     v_new = NuNv\(NuNv*v + dt*dvT.*P_temp);
    
%% there is one remaining issue with p=0 at cell boundary -> collocation uses a grid of points
...
%         colorbar;
    ...
    %% plotting and writting v and phi
        v_plot = reshape(NuNv*v_new,size_collpts,size_collpts);
        subplot(3,2,1);
        imagesc(v_plot);
        title(sprintf('iteration = %.2d',iter));
        axis square;
        colorbar;
        ...
        subplot(3,2,3);
        imagesc(reshape(bcid,lenu,lenv));
        title('bcid');
        axis square;
        colorbar;

        subplot(3,2,4);
        imagesc(reshape(P_temp,lenu,lenv));
        title('P_temp');
        axis square;
        colorbar;
