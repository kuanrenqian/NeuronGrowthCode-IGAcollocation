...
parameters = setparameters_neurite_CIL2();
...
%save('theta_original_CIL2_5.mat','theta_original');

theta1 = load('theta_original_CIL2_5.mat');
...
for i =1:Nx
    for j=1:Ny
        ...
        if(sqrt((i-center(1,1))^2+(j-center(1,2))^2)<=1.5*sqrt(seed))
            ...
            if(theta1 >= -delta_theta && theta1 <= delta_theta)
            elseif(theta1 >= -pi/3-delta_theta && theta1 <= -pi/3+delta_theta)  
            elseif(theta1 >= -7*pi/12-delta_theta && theta1 <= -7*pi/12+delta_theta)
            elseif(theta1 >= -11*pi/12-delta_theta && theta1 <= -11*pi/12+delta_theta)
            elseif(theta1 >= pi/12-delta_theta && theta1 <= pi/12+delta_theta)   
            elseif(theta1 >= 3*pi/4-delta_theta && theta1 <= 3*pi/4+delta_theta) 
            else
                theta(i,j) = 0;
            end
        end
        if(sqrt((i-center(2,1))^2+(j-center(2,2))^2)<=1.5*sqrt(seed))
            theta2 =atan2(i-center(2,1),j-center(2,2));
            if(theta2 >= pi/4-delta_theta && theta2 <= pi/4+delta_theta)
            elseif(theta2 >= -pi/4-delta_theta && theta2 <= -pi/4+delta_theta)  
...
MultipleResolution2D_neurite_CIL2
