...
parameters = setparameters_neurite_NN3();
...
center = [Nx/4,Ny/6;Nx/4,Ny/2;Nx/4,5*Ny/6;3*Nx/4,Ny/6;3*Nx/4,Ny/2;3*Nx/4,5*Ny/6];
flag_center = zeros(Nx,Ny);
for i=1:Nx
    for j=1:Ny
        if(i<=Ny/2 && j<=Nx/3)
            flag_center(i,j) = 0.1;
        elseif(i<=Ny/2 && j<=2*Nx/3 && j>=Nx/3)
            flag_center(i,j) = 0.2;
        elseif(i<=Ny/2 && j>=2*Nx/3)
            flag_center(i,j) = 0.3;
        elseif(i>=Ny/2 && j<=Nx/3)
            flag_center(i,j) = 0.4;
        elseif(i>=Ny/2 && j<=2*Nx/3 && j>=Nx/3)
            flag_center(i,j) = 0.5;
        else
...
save('theta_original_NN3.mat','theta_original');

% theta1 = load('theta_original_NN3.mat');
% theta = theta1.theta_original;

figure
imagesc(phi)

MultipleResolution2D_neurite_NN3
