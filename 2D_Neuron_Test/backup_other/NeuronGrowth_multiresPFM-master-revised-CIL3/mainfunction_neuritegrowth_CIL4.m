...
parameters = setparameters_neurite_CIL4();
...
save('theta_original_CIL4.mat','theta_original');

% theta1 = load('theta_original_CIL4.mat');
...
for i =1:Nx
    for j=1:Ny
        ...
        if(sqrt((i-center(1,1))^2+(j-center(1,2))^2)<=1.5*sqrt(seed))
            ...
            if(theta1 >= -pi/4-delta_theta && theta1 <= -pi/4+delta_theta)
            elseif(theta1 >= -pi/2-delta_theta && theta1 <= -pi/2+delta_theta)  
            elseif(theta1 >= -3*pi/4-delta_theta && theta1 <= -3*pi/4+delta_theta)
            elseif(theta1 >= 3*pi/4-delta_theta && theta1 <= 3*pi/4+delta_theta)
            elseif(theta1 >= pi/4-delta_theta && theta1 <= pi/4+delta_theta)    
            else
                theta(i,j) = 0;
            end
        end
        if(sqrt((i-center(2,1))^2+(j-center(2,2))^2)<=1.5*sqrt(seed))
            ...
            elseif(theta2 >= 3*pi/4-delta_theta && theta2 <= 3*pi/4+delta_theta)
            elseif(theta2 >= -pi/2-delta_theta && theta2 <= -pi/2+delta_theta)
            elseif(theta2 >= pi/2-delta_theta && theta2 <= pi/2+delta_theta)     
            else
                theta(i,j) = 0;
            end
...
MultipleResolution2D_neurite_CIL4
