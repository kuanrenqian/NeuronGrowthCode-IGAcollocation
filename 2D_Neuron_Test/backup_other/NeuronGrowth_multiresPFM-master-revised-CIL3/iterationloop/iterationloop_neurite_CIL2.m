...
for istep = 1:nstep
    ...
    if(iteration == iter_0)
        ...
        for i=1:Nx
            for j=1:Ny
                ...
                if(theta1>(theta_conc6(1,1)-pi/8) && theta1<=(theta_conc6(1,1)+pi/8)&& (sqrt((i-center(1,1))^2+(j-center(1,2))^2)<=3*(sqrt(seed)*dx)))
                    flag_energy(i,j) = 1;
                end
            end
        end
    ...
    %save variables
    if(mod(istep,100)==0)
        s1 = sprintf('postprocessing/phiCIL2_%d.mat',iteration);
        s2 = sprintf('postprocessing/concCIL2_%d.mat',iteration);
        s3 = sprintf('postprocessing/concCIL2_tubulin_%d.mat',iteration);
        save(s1,'phi');
        save(s2,'conc');
        save(s3,'conc_t');
    end
    save('postprocessing/iterationCIL2.mat','n_vector','iter_vector');
