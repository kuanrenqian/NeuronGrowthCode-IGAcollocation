...
for istep = 1:nstep
    ...
    if(iteration == iter_1)
        ...
        for i=1:Nx
            for j=1:Ny
                theta1 =atan2(i-center(1,1),j-center(1,2));
                if(theta1>(-pi) && theta1<=(-pi/2) && (sqrt((i-center(1,1))^2+(j-center(1,2))^2)<=10*(sqrt(seed)*dx)))
                    flag_energy(i,j) = 1;
                end
            end
        end
...
%         end
    ...
    %save variables
    if(mod(istep,100)==0)
        s1 = sprintf('postprocessing/phiCIL1_%d.mat',iteration);
        s2 = sprintf('postprocessing/concCIL1_%d.mat',iteration);
        s3 = sprintf('postprocessing/concCIL1_tubulin_%d.mat',iteration);
        save(s1,'phi');
        save(s2,'conc');
        save(s3,'conc_t');
    end
    save('postprocessing/iterationCIL1.mat','n_vector','iter_vector');
