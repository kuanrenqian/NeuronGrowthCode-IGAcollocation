...
for istep = 1:nstep
    ...
    if(iteration == iter_0)
        ...
        for i=1:Nx
            for j=1:Ny
                for kk = 1:size(center,1)
                    theta1 =atan2(i-center(kk,1),j-center(kk,2));
                    if(theta1>(theta_conc1(kk,1)-pi/6) && theta1<=(theta_conc1(kk,1)+pi/6)&& (sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=5*(sqrt(seed)*dx)))
                        flag_energy(i,j) = 1;
                    end
                    
                    if(theta1>(theta_conc2(kk,1)-pi/6) && theta1<=(theta_conc2(kk,1)+pi/6)&& (sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=5*(sqrt(seed)*dx)))
                        flag_energy(i,j) = 1;
                    end
                end
            end
    ...
    %save variables
    if(mod(istep,100)==0)
        s1 = sprintf('postprocessing/phiNN3_%d.mat',iteration);
        s2 = sprintf('postprocessing/concNN3_%d.mat',iteration);
        s3 = sprintf('postprocessing/concNN3_tubulin_%d.mat',iteration);
        save(s1,'phi');
        save(s2,'conc');
        save(s3,'conc_t');
    end
    save('postprocessing/iterationNN3.mat','n_vector','iter_vector');
