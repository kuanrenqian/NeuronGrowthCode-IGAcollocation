...
for istep = 1:nstep
    ...
    %save variables
    if(mod(istep,100)==0)
        s1 = sprintf('postprocessing/phi_ASL2_%d.mat',iteration);
        s2 = sprintf('postprocessing/conc_ASL2_%d.mat',iteration);
        s3 = sprintf('postprocessing/conc_ASL2_tubulin_%d.mat',iteration);
        save(s1,'phi');
        save(s2,'conc');
        save(s3,'conc_t');
    end
    save('postprocessing/iteration_ASL2.mat','n_vector','iter_vector');
