...
for istep = 1:nstep
        ...
        for i=1:Nx
            for j=1:Ny
                for kk = 1:size(center,1)
                    ...
                    if(theta1>(theta_conc3(kk,1)-pi/8) && theta1<=(theta_conc3(kk,1)+pi/8)&& (sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=3*(sqrt(seed)*dx)))
                        flag_energy(i,j) = 1;
                    end
                    
                    if(theta1>(theta_conc4(kk,1)-pi/8) && theta1<=(theta_conc4(kk,1)+pi/8)&& (sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=3*(sqrt(seed)*dx)))
                        flag_energy(i,j) = 1;
                    end
                end
            end
    ...
    %save variables
    if(mod(istep,10)==0)
        s1 = sprintf('postprocessing/NN4/phiNN4_%d.mat',iteration);
        s2 = sprintf('postprocessing/NN4/concNN4_%d.mat',iteration);
        s3 = sprintf('postprocessing/NN4/concNN4_tubulin_%d.mat',iteration);
        save(s1,'phi');
        save(s2,'conc');
        save(s3,'conc_t');
    end
    save('postprocessing/NN4/iterationNN4.mat','n_vector','iter_vector');
