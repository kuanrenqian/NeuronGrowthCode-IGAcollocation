...
%         end
    ...
    if(iteration == iter_0)
        ...
        for i =1:Nx
            for  j =1:Ny
                ...
                for kk= 1:size(center,1)
                    theta1 = atan2(i-center(kk,1),j-center(kk,2));
                    if(theta1>(theta_conc2(kk,1)-pi/8) && theta1<=(theta_conc2(kk,1)+pi/8))
                        flag_1(i,j) = 1;
                    elseif(theta1>(theta_conc1(kk,1)-pi/8) && theta1<=(theta_conc1(kk,1)+pi/8))
                        flag_1(i,j) = 1;
                    else
                        %calculate distance for the second longest neurite
                        if(phi(i,j)>=0.5)
        ...
        %third longest neurite and theta associated with it
        ...
        for i=1:Nx
            for j=1:Ny
                for kk = 1:size(center,1)
                    ...
                    if(theta1>(theta_conc3(kk,1)-pi/8) && theta1<=(theta_conc3(kk,1)+pi/8))
                        flag_energy(i,j) = 1;
                    end
                end
            end
        ...
        for  j =1:Ny
            ...
            if(iteration>=iter_0)
                ...
                if(flag_energy(i,j)==0)
                    if(sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=maximum_distance)
                        r = 0.1;
                        g = 0.5;
                        phi3(i,j) = 0.5;
                        delta_L(i,j) = r*conc_t(i,j) - g;
    ...
    %save variables
    if(mod(istep,100)==0)
        s1 = sprintf('postprocessing/phi_constraint3n_%d.mat',iteration);
        s2 = sprintf('postprocessing/conc_constraint3n_%d.mat',iteration);
        s3 = sprintf('postprocessing/conc_tubulin_constraint3n_%d.mat',iteration);
        save(s1,'phi');
        save(s2,'conc');
        save(s3,'conc_t');
    end
    save('postprocessing/iteration_constraint3n.mat','n_vector','iter_vector');
