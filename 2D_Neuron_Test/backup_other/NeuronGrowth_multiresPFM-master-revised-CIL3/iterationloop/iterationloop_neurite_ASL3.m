...
for istep = 1:nstep
    ...
    for kk = 1:size(center,1)
        ...
        for i=1:Nx
            for j =1:Ny
                if(phi(i,j)>=0.5 && flag_center(i,j)==0.5 && kk==1)
                    dist1(i,j) = sqrt((i-center(kk,1))^2+(j-center(kk,2))^2);
                elseif(phi(i,j)>=0.5 && flag_center(i,j)==1 && kk==2)
                    dist1(i,j) = sqrt((i-center(kk,1))^2+(j-center(kk,2))^2);
                elseif(phi(i,j)>=0.5 && flag_center(i,j)==0 && kk==3)
                    dist1(i,j) = sqrt((i-center(kk,1))^2+(j-center(kk,2))^2);
                end
            end
        end
    ...
    if(iteration == iter_0)
        ...
        for i=1:Nx
            for j=1:Ny
                theta1 =atan2(i-center(3,1),j-center(3,2));
                if(theta1>(theta_conc1(3,1)-pi/8) && theta1<=(theta_conc1(3,1)+pi/8) && (sqrt((i-center(3,1))^2+(j-center(3,2))^2)<=3*(sqrt(seed)*dx)))
                    flag_energy(i,j) = 1;
                end
            end
        end
        ...
        for  j =1:Ny
            ...
            if(iteration>=iter_0)
                ...
                if(flag_energy(i,j)==0)
                    for kk =1:3
                        if(sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=maximum_distance && kk==1 && flag_center(i,j)==0.5)
                            r = 0.1;
                            g = 0.5;
                            phi3(i,j) = 0.5;
                            delta_L(i,j) = r*conc_t(i,j) - g;
                        elseif(sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=maximum_distance && kk==2 && flag_center(i,j)==1)
                            r = 0.1;
                            g = 0.5;
                            phi3(i,j) = 0.5;
                            delta_L(i,j) = r*conc_t(i,j) - g;
                        elseif(sqrt((i-center(kk,1))^2+(j-center(kk,2))^2)<=maximum_distance && kk==3 && flag_center(i,j)==0)
                            r = 0.1;
                            g = 0.5;
                            phi3(i,j) = 0.5;
                            delta_L(i,j) = r*conc_t(i,j) - g;
    ...
    %save variables
    if(mod(istep,100)==0)
        s1 = sprintf('postprocessing/phiASL3_%d.mat',iteration);
        s2 = sprintf('postprocessing/concASL3_%d.mat',iteration);
        s3 = sprintf('postprocessing/concASL3_tubulin_%d.mat',iteration);
        save(s1,'phi');
        save(s2,'conc');
        save(s3,'conc_t');
    end
    save('postprocessing/iterationASL3.mat','n_vector','iter_vector');
