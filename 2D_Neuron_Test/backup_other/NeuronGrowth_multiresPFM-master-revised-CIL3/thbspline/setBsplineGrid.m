...
%% Knotvectors
for lev = 1:maxlev,
    knotvectorU{lev,1} = [dx.*ones(1,pU), dx:dx*0.5^(lev-1)*(Nx-1)/nelemx:(Nx)*dx, dx*(Nx).*ones(1,pU)];
    ...
    knotvectorV{lev,1} = [dy.*ones(1,pU), dy:dy*0.5^(lev-1)*(Ny-1)/nelemy:(Ny)*dy, dy*(Ny).*ones(1,pV)];
