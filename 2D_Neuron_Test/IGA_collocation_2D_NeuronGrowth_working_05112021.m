...
%         imagesc(conc_plot(2:end-1,2:end-1));
        ...
        subplot(5,2,6);
%         imagesc(a_plot(2:end-1,2:end-1));
        imagesc(a_plot);
...
%         colorbar;
        ...
        phydx = N1uNv*phiK;
        phidy = NuN1v*phiK;
        phydx = reshape(phydx,lenu,lenv);
        phidy = reshape(phidy,lenu,lenv);
        for j = 1:lenv
            for i = 1:floor(lenu/2)
                phydx(j,i) = abs(phydx(j,i));
                phidy(i,j) = abs(phidy(i,j));
            end
            for i = floor(lenu/2):lenu
                phydx(j,i) = -abs(phydx(j,i));
                phidy(i,j) = -abs(phidy(i,j));
            end
        end
...
%         [phidy,phidx]=gradient_mat_imf(fullPhi,1,1);
        pdy_plot = full(reshape(phidy,lenu,lenv));
...
%         imagesc(pdy_plot(2:end-1,2:end-1));
        ...
        pdx_plot = full(reshape(phydx,lenu,lenv));
        subplot(5,2,10);
...
%         pplot = reshape(atan2(phidy,phidx),lenu,lenv);
        pplot = reshape(smooth(smooth(full(atan2(phidy,phydx)))),lenu,lenv);
        subplot(5,2,8);
